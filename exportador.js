...
export function exportarParaCSV(dados, nomeArquivo = "obras_exportadas") {
  ...
  const dadosExportacao = dados.map(obra => ({
    "Objeto": obra.descricaoContrato || obra.descricaoGeometria || obra.informacoesAdicionais || obra.enderecoFallback || `Obra #${obra.obraId}`,
...
export function exportarParaExcel(dados, nomeArquivo = "obras_exportadas") {
  ...
  for (const obra of dados) {
    const objeto = escapeHtml(obra.descricaoContrato || obra.descricaoGeometria || obra.informacoesAdicionais || obra.enderecoFallback || `Obra #${obra.obraId}`);
